...
public:
	...
	inline void set_db_inst(TlpDB db, int timeout)
	{
		datebase_ = db;
		timewait_db_alive_ = timeout;
	}
...
#endif
    {
        SL_Seda_StageThread<TSedaStageHandler, TlpDB> *stage_thread = (SL_Seda_StageThread<TSedaStageHandler, TlpDB> *)arg;
		...
		TlpDB	db_inst				= stage_thread->datebase_;
            ...
            {//timer_event_flag = false and idle_event_flag = false
                while (1)
                {
                    if (stage_thread->event_queue_active_->pop(&event) > 0)
                    {
                        ...
                        {
                            stage_thread->stage_handler_.handle_event(event, db_inst, NULL);
                        }
                        else
                        {
                    ...
                    else
                    {
						...
						if (current_clock_ms - idle_last_clock_ms >= stage_thread->timewait_db_alive_)
						{
							int flag = SP_DB_CommonAPI::Detect(stage_thread->datebase_);

							printf("SP_DB_CommonAPI::Detect flag = %d", flag);
            ...
            {///timer_event_flag = false and idle_event_flag = true
                while (1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            stage_thread->stage_handler_.handle_event(&idle_event, db_inst, NULL);
                            idle_last_clock_ms = current_clock_ms;
                        }
        ...
        else
        {
            ...
            {//timer_event_flag = true and idle_event_flag = false
                while (1)
                {
                    ...
                    {
                        current_clock_ms = SL_Socket_CommonAPI::util_process_clock_ms();
                        stage_thread->check_timers(current_clock_ms, &timer_expire_event);//wanghuan debug
                        timer_event_count = 0;
