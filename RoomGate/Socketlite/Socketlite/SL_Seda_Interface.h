...
public:
    ...
    virtual int  handle_event(const SL_Seda_Event *event, void *db, void *cache) = 0;
};
...
public:
    ...
    virtual int open(int thread_number, uint queue_max_size, uint event_max_len, uint timedwait_interval_us, bool timedwait_signal, int type, int timewait_db_alive) = 0;
    virtual int close() = 0;
