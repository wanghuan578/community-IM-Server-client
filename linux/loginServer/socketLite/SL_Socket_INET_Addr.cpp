...
#if 0
int SL_Socket_INET_Addr::get_ip_addr_ex(char *ip_addr, int len)
{
	struct addrinfo hints = {0};
	struct addrinfo *ret = NULL;
	char host[256];
	...
	if (NULL != host)
	{
		...
		int error = getaddrinfo(host, 0, &hints, &ret);
		...
		if (addr.sa_family == AF_INET)
		{
			...
			sprintf_s(ip_addr, len, "%d.%d.%d.%d", in4->sin_addr.S_un.S_un_b.s_b1, in4->sin_addr.S_un.S_un_b.s_b2, in4->sin_addr.S_un.S_un_b.s_b3, in4->sin_addr.S_un.S_un_b.s_b4);
		}
		else
		{
			sockaddr_in6 *in6 = (sockaddr_in6*)&addr;
			//sprintf_s(ip_addr, len, "%d.%d.%d.%d", in6->sin_addr.S_un.S_un_b.s_b1, in4->sin_addr.S_un.S_un_b.s_b2, in4->sin_addr.S_un.S_un_b.s_b3, in4->sin_addr.S_un.S_un_b.s_b4);
		}
	}
