...
public:
    ...
    inline static int message_to_transmit(char *msg, 
        int len, 
        int to_dest_id,
        SL_Seda_RpcTransmitProc proc, 
        SL_Seda_RpcTransmitEvent *event)
    {
        char *sl_msg = new char[len];
        if (NULL == sl_msg)
        {
            return -1;
        }
        ...
        {
	    memcpy(sl_msg, msg, len);
            event->to_dest_id = to_dest_id;
            event->transmit_proc = proc;
            event->transmit_message = sl_msg;
	    event->message_len = len;
            return 0;
        }
